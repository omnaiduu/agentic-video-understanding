...
- search: we hybrid-search the Whisper transcript (keyword + meaning). Put the phrase in query, or null to use the user question. We return at most 8 {t, text} hits as text, never the whole talk. We ignore answer. If the transcript is not ready, we say so; look or listen, then answer. Do not only apologize.
...
- search_slides: we MaxSim-search unique slides (ColQwen2.x patches). Put the phrase in query, or null to use the user question. We return at most 8 {t, score, slide_id} hits as text, never two hours of frames. Scores are not the answer; look at a hit to read the real frame. We ignore answer. Search this book at most once, then look at the top hit (under 2 seconds), then answer. After look, describe the pixels. If the frame is a red screen or a different heading, say that — do not copy a price from the question onto the wrong slide. If the slide book is not ready, we say so; timed look still works — look, then answer. Do not use this for birds or red lights (that is search_visual) or spoken words (that is search).
