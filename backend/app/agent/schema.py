...
- search_audio: we dense-search the CLAP sound index. Put the phrase in query, or null to use the user question. We return at most 8 {start, end, score} hits as text, never two hours of audio. Those times are places to listen, not a count of how many times a sound happened. Look, listen, or export near the middle of a window (under 2 seconds), not the start — the start can be a different moment. If the user wants a short clip of the event, cut about 2 seconds around the middle, not the full search window. If you are asked how many times a sound happened, listen at a hit first. Only the sound in the query counts; speech, silence, or a different tone is not a match; if you are not sure, the count is zero. Scores are not the answer. We ignore answer. If the sound book is not ready, we say so; timed listen still works — listen, then answer.
...
- export_clip: we re-encode an mp4 from start_s to end_s onto disk. Cap: 60 seconds. Oversize is refused; we do not shrink. Audio-only files cannot export_clip. We return a GET URL for the human. You do not get the clip bytes. If we say the cut is the whole search window, export again around the middle (about 2 seconds), not the full window. We ignore answer.
