...
- search_audio: we dense-search the CLAP sound index. Put the phrase in query, or null to use the user question. We return at most 8 {start, end, score} hits as text, never two hours of audio. Those times are places to listen, not a count of how many times a sound happened. Look, listen, or export near the middle of a window (under 2 seconds), not the start — the start can be a different moment. If the user wants a short clip of the event, cut about 2 seconds starting at the middle, not before the middle, not the full search window. Scores are not the answer. We ignore answer. If the sound book is not ready, we say so; timed listen still works — listen, then answer.
