...
4. **One index cannot cover all questions.** Speech, pictures, sounds, and **printed slides** are four channels. Image embeddings do not find a bird **chirp**. Transcript does not find a silent red light. SigLIP does not find **Pro $99** on a busy slide.
...
6. **CLIP-class models find; they do not answer.** Retrieval is a map. Gemma looks at real frames/audio and answers. Counting is done **in code** on hits, not by the LLM “remembering 847 claps.” ColQwen2.x is the same rule: it finds the slide; Gemma reads the frame.
...
- Ask talk, visual, **on-screen slide text** (“which slide had Pro $99?”), and sound questions
- Get timestamped answers grounded in a short rewatch
- Optionally get a **link to a cut clip or audio slice**
- Second question on the same file does **not** re-run Whisper/SigLIP/CLAP/ColQwen
